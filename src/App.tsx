    ...
    <>
      ...
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col items-center p-4 transition-colors duration-200 pt-20">
        ...
        <header className="w-full max-w-2xl flex flex-col items-center mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-300 text-center mb-1">{t.subtitle}</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 text-center">{t.brands}</p>
        </header>
        <form onSubmit={handleParse} className="w-full max-w-md bg-white dark:bg-gray-800 rounded shadow p-6 flex flex-col gap-4">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? 'Processing TeraBox Link...' : t.inputLabel}
          </button>
        </form>
        {error && <div className="mt-4 text-red-600 dark:text-red-400">{t.error}</div>}
        {fileInfo && (
          <div className="mt-6 bg-white dark:bg-gray-800 rounded shadow p-6 w-full max-w-md">
            ...
            {fileInfo.fileSize && (
              <div className="mb-2">
                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{t.fileSize}</span> 
                <span className="ml-1 text-gray-900 dark:text-white">{fileInfo.fileSize}</span>
              </div>
            )}
            <div className="mb-2">
              <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{t.fileType}</span> 
